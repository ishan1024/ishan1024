...
struct PuzzleState
{
    int puzzle[N][N];
    int zeroRow, zeroCol;
    int g; // number of moves from the start( actual cost from the start node to the current node)
    int h; // heuristic cost to the goal(approximate estimate of the cost)
...
void printPuzzle(const PuzzleState &state) // Displays the current puzzle state.
{
    for (int i = 0; i < N; i++)
    {
        for (int j = 0; j < N; j++)
...
{ // Ensures that a tile move doesn’t go out of bounds.
...
{ // Represents directions: left, right, up, down.
    ...
    for (int k = 0; k < 4; k++)
    {
        ...
        { // Creates new valid puzzle states after moving the blank tile.
...
{ // Initialize the queue with the initial state. visited keeps track of explored states.
    ...
    while (!pq.empty())
    { // Goal check: if reached, print and break
        ...
        vector<PuzzleState> nextStates = generateNextStates(current); // Generate next valid states and push them to the queue if not visited.
